...
-- total de valores de emprestimo
-- total de valores de deposito
-- caixa, 2000, 100, 200

select  a.agencia_nome,sum(dv.valor) v_devedores
from agencia a
...
select  a.agencia_nome,sum(d.valor) v_depositos
from agencia a
left join conta c
	on c.agencia_nome=a.agencia_nome
left join depositante d
...
select dev.agencia_nome, dev.v_devedores, dep.v_depositos
 from (
	select  a.agencia_nome,sum(dv.valor) v_devedores
	from agencia a
	left join emprestimo e 
...
on dev.agencia_nome = dep.agencia_nome


select a.agencia_nome,a.ativos,
(
	select  sum(dv.valor) v_devedores
	from emprestimo e 
	inner join devedor dv
		on dv.emprestimo_numero = e.emprestimo_numero
	where e.agencia_nome = a.agencia_nome
	group by e.agencia_nome
) as v_devedores,
(
	select sum(d.valor) v_depositos
	from conta c
	inner join depositante d
		on d.conta_numero = c.conta_numero
	where c.agencia_nome = a.agencia_nome
	group by c.agencia_nome
) as v_depositos
from agencia a

select  e.agencia_nome,sum(dv.valor) v_devedores
from emprestimo e 
inner join devedor dv
	on dv.emprestimo_numero = e.emprestimo_numero
group by e.agencia_nome
...
select  c.agencia_nome,sum(d.valor) v_depositos
from conta c
inner join depositante d
	on d.conta_numero = c.conta_numero
group by c.agencia_nome
...
-- listar o cliente(nome, cidade)
-- total em emprestimo
-- total em depositos

select c.cliente_nome, c.cliente_cidade, 
( 
	select sum(d.valor) from devedor d
	where d.cliente_nome = c.cliente_nome
) as t_emprestimo,
(
	select sum(d.valor) from depositante d
	where d.cliente_nome = c.cliente_nome
) as t_deposito
from cliente c

-- regras para utilizar Subquery
-- quando for usar no IN
-- ex: select * from cliente 
--     where cliente_nome in (subquery)

--  deve retornar uma coluna e 0 ou n linhas

-- quando for usar em um = (igual)
-- ex: select * from cliente 
--     where cliente_nome = (subquery)
-- deve retornar uma coluna e 0 ou 1 linha

-- quando for usar no lugar de uma tabela
-- ex select * from (subquery) x
-- 

-- quando for usar no lugar de uma colulna
-- ex: select (subqery) as coluna from tabela

-- deve retornar uma coluna 0 ou 1 linha
